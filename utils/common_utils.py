...
def save_checkpoint(prefix: str,
    ...
    if isinstance(supcluster_model, DistributedDataParallel):
        supcluster_model = supcluster_model.module
    if isinstance(supcluster_cam_model, DistributedDataParallel):
        supcluster_model = supcluster_cam_model.module
    ...
    torch.save(
        {
            ...
            'supcluster_model_state_dict': supcluster_model.state_dict(),
            'supcluster_optimizer_state_dict': supcluster_optimizer.state_dict() if (not model_only) else None,
            'supcluster_cam_model_state_dict': supcluster_cam_model.state_dict(),
            'supcluster_cam_optimizer_state_dict': supcluster_cam_optimizer.state_dict() if (not model_only) else None,
            'best': best_value,
        }, model_name)
